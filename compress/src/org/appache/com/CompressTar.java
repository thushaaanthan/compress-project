        ...
        try {
            ...
            if (fileObj.exists()) {
                if (fileObj.getType() == FileType.FOLDER) {
                    ...
                    try {
                        outputStream = new TarArchiveOutputStream(destObj.getContent().getOutputStream());
                        fileIn = fileObj.getContent().getInputStream();
                        TarArchiveEntry TarArchiveEntry = new TarArchiveEntry(fileObj.getName().getBaseName());
    ...
    private void addToTar(FileObject fileObject, FileObject file, TarArchiveOutputStream outputStream) {
        ...
        try {
            ...
            String entry = name.substring(fileObject.getName().toString().length() + 1,
                    name.length());
            TarArchiveEntry TarArchiveEntry = new TarArchiveEntry(entry);
            outputStream.putArchiveEntry(TarArchiveEntry);
