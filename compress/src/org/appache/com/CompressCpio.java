...
/**
 * Created by TSA10 on 1/12/17.
 */


public class CompressCpio {
        ...
        try {
            ...
            if (fileObj.exists()) {
                if (fileObj.getType() == FileType.FOLDER) {
                    ...
                    try {
                        outputStream = new CpioArchiveOutputStream(destObj.getContent().getOutputStream());
                        fileIn = fileObj.getContent().getInputStream();
                        CpioArchiveEntry CpioArchiveEntry = new CpioArchiveEntry(fileObj.getName().getBaseName());
                        outputStream.putArchiveEntry(CpioArchiveEntry);
    ...
    private void addToZip(FileObject fileObject, FileObject file, CpioArchiveOutputStream outputStream) {
        ...
        try {
            ...
            String entry = name.substring(fileObject.getName().toString().length() + 1,
                    name.length());
            CpioArchiveEntry CpioArchiveEntry = new CpioArchiveEntry(entry);
            outputStream.putArchiveEntry(CpioArchiveEntry);
